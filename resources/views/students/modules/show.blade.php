<x-layout>
    ...
    <x-containers.main>
                ...
                @foreach($comments as $comment)
                    <div class="space-y-1">
                        <h2 class="text-blue-600">
                            @if($comment->user->profile_type === 'App\Models\TeacherProfile')
                                <span class="bg-blue-900 text-white text-sm py-0.5 px-5 rounded-full">Teacher</span>
                            @else
        ...
        <div class="mt-5">
            <form action="{{ route('student.comments.store', [$schedule->id, $module->id]) }}" method="POST">
                @csrf
